#' @export
BarPlotLF<-function(LF,yrs=2005:2016,LFyrs=yrs,bins=seq(0,220,5),filen='LF.pdf',rows=length(yrs),pdf=T,xl,rel=T,mean.line=F,ylp=0.1,ymax,LS=95,sample.size=NULL,recline=NULL,wd=10,ht=12,bx=F,xlab="Carapace Length (mm)",ylab="Mean N / standard tow",...){
    ...
    if(!is.null(LS)){
        ...
        if(ncol(LS)==1)LS=matrix(LS,length(yrs),length(LF))
    }
    ...
    #browser()
        ...
        for(y in which(yrs==LFyrs)){
            ...
            if(y==max(which(yrs==LFyrs))){
                axis(1)
                if(mean.line){
                    lines(tmp$mids,colMeans(LF[[i]][-nrow(LF[[i]]),]),lwd=1)
            ...
            mtext(as.character(LFyrs[y]), 3, -3, adj=ylp,outer = F,cex=1)
